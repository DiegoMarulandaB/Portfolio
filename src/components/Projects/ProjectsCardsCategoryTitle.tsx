...
const ProjectsCardsCategoryTitle: React.FC<ProjectCardsProps> = ({
  ...
  >
    <Slide offset="-200px 0px -200px 0px">
      <div className="flex flex-col items-center text-center rounded-lg bg-slate-50 dark:bg-[#2b2d39] shadow-lg mx-auto mt-8">
        <div className="overflow-hidden rounded-t-lg">
          <a href={href} target="_blank" rel="noopener noreferrer">
            <img
              ...
              src={ImageHref}
              alt={title}
              className="img-projects-cards w-full h-auto object-cover"
            />
          </a>
        </div>
        <div className="px-4 py-4">
          <h2 className="heading-2-projects-category">{capitalize(category)}</h2>
          <h3 className="span-projects-title">{title}</h3>
          <p className="paragraph-project-description">{description}</p>
          {/* <div className="flex items-start">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="button-card-projects-github buttons  inline-block text-center rounded-lg  px-2.5 py-2.5 me-1 mb-4"
